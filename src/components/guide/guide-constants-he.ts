...
Sub-options עוקבים אחר אותה הסכמה \`opts-{depId}=opt1,opt2\` כמו בנתיב ה-backend; ה-servlet filter הקיים \`InitializrWebConfiguration\` מאכלס את \`ProjectOptionsContext\` לכל בקשה שמגיעה לאפליקציה, כך שנקודות הקצה של ה-frontend מקבלות sub-options בחינם.`,
  ...
  // ─────────────────────────────────────────────────────────────────
  // 3. מחולל זוגי (Backend + Frontend Monorepo)
  // ─────────────────────────────────────────────────────────────────
  {
    id: 'paired-generator',
    title: 'מחולל זוגי',
    ...
    topics: [
      {
        id: 'paired-what',
        title: 'מה זה המחולל הזוגי?',
        description: 'בקשת JSON אחת שמייצרת Spring Boot ב-Backend ו-React/Vite ב-Frontend, מחוברים מראש כדי לדבר אחד עם השני.',
        content: `### מטרה
לשונית **Paired** במסך הראשי מחוללת **את שני חצאי הסטאק** בבקשה אחת. התשובה היא zip של monorepo עם \`backend/\`, \`frontend/\`, \`README.md\` ב-root, \`docker-compose.yml\` עובד, ו-\`.menora-init.json\` שמתאר את שני החצאים ומפרט SHA-256 לכל קובץ.

זה הזווית הייחודית מול \`start.spring.io\` + \`create-vite\`: אותו backend מחולל את שני החצאים, והם מחוברים אוטומטית לדבר על \`localhost\` מהרגע שמריצים \`unzip\`.

### נקודות קצה
- \`POST /starter-paired.zip\` — ZIP בינארי של monorepo
- \`POST /agent/scaffold/paired\` — עטיפת JSON (עץ קבצים utf-8 / base64 + paired manifest), לסוכנים

### גוף הבקשה
\`\`\`json
{
  "backend":  { /* אותו מבנה כמו WizardStarterRequest */ },
  "frontend": { /* PairedFrontendRequest — projectName, deps, apiBaseUrl, ... */ }
}
\`\`\`

ניתן להשמיט כל חצי; הצד החסר פשוט מדולג. הלשוניות הייעודיות **Backend** ו-**Frontend** עדיין קיימות לזרימות פרויקט-יחיד עשירות יותר (אשפים, multi-module, presets, color palettes) — לשונית Paired מינימלית בכוונה.`,
        callouts: [
          {
            type: 'info',
            text: 'נקודת הקצה הזוגית מרכיבה את שני הצינורות הקיימים — לא מציגה מחולל שלישי. PairedStarterController קורא ל-ProjectGenerationInvoker עבור BE ואז ל-FrontendProjectGenerator עבור FE, ממזג את שני העצים תחת קידומות backend/ ו-frontend/, ומוסיף את קבצי ה-root.'
          }
        ]
      },
      {
        id: 'paired-auto-wiring',
        title: 'איך שני החצאים מחוברים אוטומטית',
        description: 'ברירות מחדל בין סוגי הפרויקטים מופעלות כשיש שני חצאים.',
        content: `### מה מתחבר
| חיווט | מנגנון |
|---|---|
| FE יודע את ה-URL של ה-BE | \`frontend.apiBaseUrl\` ברירת מחדל \`http://localhost:8080\`. נכתב ל-\`frontend/.env.development\` כ-\`VITE_API_BASE_URL\` ולבלוק \`server.proxy\` ב-\`vite.config.ts\` כדי ש-\`/api/*\` יועבר ל-backend בפיתוח |
| BE מקבל את המקור של FE | תת-אפשרות \`paired-cors\` מתווספת אוטומטית ל-dep \`web\` של ה-BE — מייצרת \`CorsConfig.java\` שמאפשרת \`http://localhost:5173\` ל-\`/api/**\` |
| חוזה OpenAPI משותף | אם ה-BE שולח OpenAPI spec וה-FE בוחר ב-\`api-client-openapi\`, אותו spec נוחת ב-\`frontend/openapi.yaml\` |
| הקשר זיווג ב-README | \`README.md\` בשורש מזכיר את שני שמות הארטיפקטים ואת ה-\`apiBaseUrl\` |

### ברירות מחדל ומעקות בטיחות
- קובץ ה-CORS מתווסף רק כש-\`web\` נמצא ב-BE.
- \`frontend.apiBaseUrl\` מפורש דורס את ברירת המחדל — שימושי כשמחברים מול backend פיתוח מרוחק.
- \`FrontendProjectDescription.setApiBaseUrl\` דוחה כל מה שאינו URL \`http(s)://\` עם תווים בטוחים, כך שהערך יכול להישלב בקבצי הגדרה ללא escaping.`,
      },
      {
        id: 'paired-openapi',
        title: 'לקוח טיפוסי מ-OpenAPI',
        description: 'שימוש חוזר ב-spec שמניע BE codegen גם כדי לחולל לקוח FE טיפוסי.',
        content: `### Dep api-client-openapi
לקטלוג ה-FE נוספה קבוצת **API Integration** עם entry יחיד: \`api-client-openapi\`. בחירה בו מוסיפה:
- \`openapi-typescript\` ל-\`devDependencies\`
- סקריפט npm \`gen:api\` שמריץ \`openapi-typescript openapi.yaml -o src/shared/api/generated/schema.ts\`
- \`src/shared/api/generated/.gitkeep\` כדי שתיקיית הפלט תהיה קיימת לפני הריצה הראשונה
- \`src/shared/api/README.md\` קצר שמתעד את ה-workflow
- כלל תאימות \`api-client-openapi RECOMMENDS data-tanstack-query\`

### ה-Spec עצמו
ה-OpenAPI spec נוחת ב-\`frontend/openapi.yaml\` רק אם שני אלה מתקיימים:
1. \`api-client-openapi\` נמצא ב-FE deps
2. spec לא ריק נמצא ב-\`OpenApiSpecContext\`

בזרימה הזוגית, ה-spec מגיע מ-\`backend.specByDep.openapi\` — אותו שדה שאשף ה-OpenAPI של ה-BE כבר משתמש בו כדי להניע יצירת controllers / DTOs.

בחירה ב-dep ללא spec לא מזיקה: הסקריפטים וה-devDep עדיין נשלחים, ניתן לכתוב \`openapi.yaml\` ידנית ולהריץ \`gen:api\`.

### לקוח טיפוסי מחולל מראש
כש-spec מסופק, ה-zip הזוגי כולל לקוח טיפוסי פעיל ב-\`src/shared/api/generated/\` — מופק על ידי OpenAPI → TS codegen ב-Java טהור, ללא צורך ב-Node. \`tsc --noEmit\` עובר מיד אחרי החילוץ. הקבצים:

- \`schema.ts\` — \`export type\` לכל component schema. \`oneOf\` עם \`discriminator\` הופך ל-tagged union (ניתן לצמצום על התג); \`allOf\` הופך לחיתוך (intersection); \`nullable\` הופך ל-\`T | null\`
- \`paths.ts\` — מפת path/method טיפוסית. תגובות מחולקות ל-\`success\` (2xx) ול-\`error\` (השאר + \`default\`); JSDoc נפלט עבור \`summary\` / \`description\` / \`@deprecated\`
- \`errors.ts\` — טיפוסים מותנים \`SuccessBody<P, M>\` / \`ErrorBody<P, M>\`, מעטפת \`ApiError<P, M>\`, ושומר ריצה \`isApiError(x)\`
- \`client.ts\` — \`createApiClient()\` עם \`request()\` (\`Response\` גולמית) ו-\`requestJson<P, M>()\` (מחזיר את גוף ההצלחה הממוקד; זורק \`ApiError\` ב-non-2xx)
- \`hooks.ts\` *(רק כאשר \`data-tanstack-query\` נבחר גם הוא)* — \`useXxx\` ל-GETs (\`useQuery\`) ו-\`useXxxMutation\` ל-POST/PUT/PATCH/DELETE
- \`msw.ts\` *(רק כאשר \`test-vitest-rtl\` נבחר גם הוא)* — handlers של MSW v2 שנזרעים מ-\`example\` ב-spec → \`example\` ב-schema → stub מסונתז

### תת-אפשרות react-query-hooks
בחירה בתת-אפשרות זו מייצרת \`orval.config.ts\` שמוגדר להפיק React Query hooks מ-\`openapi.yaml\`. \`orval\` עצמו לא מותקן אוטומטית — המשתמשים מתקינים ידנית עם \`pnpm add -D orval\` ואז \`npx orval\`.`,
      },
      {
        id: 'paired-using',
        title: 'שימוש בלשונית Paired',
        description: 'שני טורים, כפתור Generate אחד, OpenAPI spec אופציונלי.',
        content: `### מבנה
לשונית Paired מחלקת את המסך לשניים:
- **טור Backend** — Artifact ID, package name אופציונלי, Boot version, ובוחר תלויות פשוט מ-\`/metadata/client\`
- **טור Frontend** — שם פרויקט, גלולת package manager, ובוחר תלויות פשוט מ-\`/frontend/metadata\` (כולל את הקבוצה החדשה **API Integration**)

מתחת לשני הטורים:
- שדה **API Base URL** — ריק = ברירת מחדל \`http://localhost:8080\`
- אזור טקסט **OpenAPI spec** — הדבק spec כדי להניע גם BE codegen וגם לקוח FE טיפוסי. כשממולא, ה-dep \`openapi\` של ה-BE מתווסף אוטומטית אם חסר.

כפתור **Generate Paired Zip** שולח את ה-JSON הבנוי ל-\`/starter-paired.zip\` ומוריד את ה-blob. אין שמירה ב-localStorage / URL ב-MVP הזה; לשמירה / שיתוף, השתמשו בלשוניות **Backend** ו-**Frontend** הייעודיות.

### הערות סכמה
שלב 2 של עבודת הזיווג הציג את תת-האפשרות \`paired-cors\` ב-dep \`web\` של ה-BE. שלב 3 הוסיף את ה-dep \`api-client-openapi\` של ה-FE. מיגרציית Flyway \`V7\` מרחיבה את \`build_customization.version\` מ-\`VARCHAR(50)\` ל-\`VARCHAR(2000)\` — העמודה הזו מפורשת מחדש על ידי \`ADD_NPM_SCRIPT\` / \`ADD_VITE_PLUGIN\` כפקודה / ביטוי קריאה, כאשר 50 תווים היו צרים מדי לסקריפטים מציאותיים.`,
      },
    ]
  },
