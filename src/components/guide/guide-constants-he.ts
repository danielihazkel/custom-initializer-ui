...
Sub-options עוקבים אחר אותה הסכמה \`opts-{depId}=opt1,opt2\` כמו בנתיב ה-backend; ה-servlet filter הקיים \`InitializrWebConfiguration\` מאכלס את \`ProjectOptionsContext\` לכל בקשה שמגיעה לאפליקציה, כך שנקודות הקצה של ה-frontend מקבלות sub-options בחינם.`,
  ...
  // ─────────────────────────────────────────────────────────────────
  // 2.5 מחולל CRUD מלא (Fullstack)
  // ─────────────────────────────────────────────────────────────────
  {
    id: 'fullstack-generator',
    title: 'מחולל CRUD מלא',
    icon: 'dynamic_form',
    topics: [
      {
        id: 'fs-what',
        title: 'מה זה המחולל המלא?',
        description: 'מגדירים ישויות ומקבלים backend ב-Spring Boot + frontend ב-React מחוברים יחדיו בקובץ ZIP אחד.',
        content: `### מטרה
לשונית **Fullstack** מקבלת רשימת ישויות מוגדרות-משתמש (שם + שדות פרימיטיביים) ופולטת ZIP בודד הכולל backend עובד של Spring Boot (\`backend/\`) ו-frontend עובד של React (\`frontend/\`) — כבר מחוברים יחדיו. כל ישות הופכת לסט של \`@Entity\` + \`Repository\` + \`Dto\` + \`Service\` + \`@RestController\` ב-backend (עם דפדוף, מיון וחיפוש בצד השרת) ולעמוד CRUD עם Table ו-Form ב-frontend.

רוב המערכות הפנימיות של מנורה שנכתבות מחדש הן "frontend עם טבלאות ופעולות CRUD". המחולל הזה הופך כל מערכת חדשה ממשימה של ימי דבק לכמה דקות של לחיצות.

### למה מחולל שלישי?
מחולל ה-Backend פולט Spring Boot בלבד. מחולל ה-Frontend פולט React בלבד. אף אחד מהם לא יכול ללולל מעל רשימת ישויות שנשלחה מהמשתמש כדי לייצר N עותקים של \`<Entity>Controller.java\` + \`<Entity>Page.tsx\` סביב אותו מודל דומיין. Fullstack סוגר את הלולאה הזו:

- **Backend** עובר דרך ה-pipeline הסטנדרטי של Spring Initializr. \`EntityScaffoldContributor\` חדש (רשום ב-\`META-INF/spring.factories\`) אוסף את רשימת הישויות מ-ThreadLocal בשם \`EntityDefinitionContext\` ומרנדר קבצים פר-ישות.
- **Frontend** מרונדר inline מחוץ ל-pipeline. ה-framework של Initializr בנוי סביב פרויקטים של Java ולא מכיר Vite, אז \`FullstackRenderer\` כותב את קבצי ה-Mustache ישירות תחת \`frontend/\`.
- שני הפלטים נוחתים בתיקייה זמנית, מקבלים \`README.md\` ו-\`.gitignore\` ב-root, ואז נארזים ל-ZIP ומוזרמים.

### מה נתמך (v1)
- **טיפוסי שדה פרימיטיביים בלבד**: String, Long, Integer, Boolean, LocalDate, LocalDateTime, BigDecimal, Enum (מרונדר כמחלקה מקוננת על הישות).
- **אין קשרים** — שומרים את זה פשוט. \`referencesEntity\` מתוכנן ל-v3.
- **נקודות קצה CRUD פתוחות** — \`@CrossOrigin\` על כל controller כדי שה-Vite dev server על :5173 יוכל לפנות אל :8080 בלי proxy. הוסיפו את התלות \`security\` ב-picker אם צריך אימות.`,
        callouts: [
          {
            type: 'info',
            text: 'לשונית Fullstack בלתי תלויה בלשוניות Backend ו-Frontend. היא קוראת ל-POST `/starter-fullstack.zip` עם גוף JSON ומורידה את התשובה כ-`{artifactId}.zip`.'
          }
        ]
      },
      {
        id: 'fs-using',
        title: 'שימוש בלשונית Fullstack',
        description: 'שלב-אחר-שלב: מאשף ריק ועד ZIP מורד שעובד.',
        content: `### שלב 1 — מטא-דאטה של הפרויקט
\`groupId\`, \`artifactId\`, \`packageName\`, גרסת Java. שדות סטנדרטיים של Spring Initializr — ממולאים מראש בברירות מחדל שפויות.

### שלב 2 — סטים של תבניות
בחרו את סט התבניות ל-backend ול-frontend. שניים נשלחים בהתקנה: **spring-jpa-crud** ו-**react-tailwind-crud**. מנהלים יכולים לשכפל או להוסיף עוד תחת Config → Entity CRUD.

### שלב 3 — תלויות
מסומנים מראש לפי ברירות המחדל של סט ה-backend הנבחר (data-jpa, web, h2, validation, actuator עבור \`spring-jpa-crud\` המקורי). אפשר לסמן או לבטל הכל — המחולל מכבד בדיוק את הבחירה הסופית שלכם. שום דבר לא נכפה.

### שלב 4 — ישויות
עורך הישויות מציג כל ישות ככרטיס עם שדה שם וטבלת שדות פנימית. לכל שדה יש:
- **Name** — מזהה Java, הופך לעמודה ולשדה TS.
- **Type** — String, Long, Integer, Boolean, LocalDate, LocalDateTime, BigDecimal, Enum.
- **PK** / **Gen** / **Req** / **Uniq** — מתגי דגלים ל-primary key, generated, required, unique.
- **Length** — רק עבור String (הופך ל-\`@Column(length=…)\` + רמז UI ב-TS).
- **Enum values** — רק עבור Enum (הופך ל-enum מקוננת על הישות).

שני כפתורי נוחות מעל רשימת הישויות:
- **Import from DDL** — להדביק CREATE TABLE, לבחור דיאלקט, לקבל את העורך ממולא מראש.
- **Add entity** — מוסיף כרטיס ריק.

### שלב 5 — Generate
כפתור Generate שולח POST ל-\`/starter-fullstack.zip\` ומזרים את התוצאה. לפתוח את ה-ZIP → \`cd backend && ./mvnw spring-boot:run\` → לפתוח את ה-Vite dev server על :5173.`,
        callouts: [
          {
            type: 'tip',
            text: 'האשף מטעין מחדש את ה-dep picker בכל פעם שמחליפים את סט ה-backend — החלפת סט נותנת את ברירות המחדל של אותו סט, מסומנות מחדש. כדי להחיל מחדש ברירות מחדל אחרי עריכה ידנית, לחצו "Reset to defaults" ב-dep picker.'
          }
        ]
      },
      {
        id: 'fs-pages',
        title: 'עמודי הפרונטאנד',
        description: 'לעצב את הפרונטאנד שנוצר כעמודים — דשבורדים, רשימות, לשוניות, אב-פרט, רשומות, דוחות ואשפים — במקום המעטפת הקלאסית הקבועה.',
        content: `### מעטפת קלאסית או פריסה
בלי פריסה, האפליקציה שנוצרת מכילה דשבורד ועמוד רשימה אחד לכל ישות. הסעיף **Frontend pages** (מתחת ל-Entities) מחליף זאת בעמודים משלכם. **Start from my entities** זורע דשבורד ורשימה לכל ישות, הדוגמאות מגיעות עם פריסות ללמוד מהן, ו-**Use classic layout** מוותר על הפריסה (ניתן לביטול).

### סוגי העמודים
- **Dashboard** — ווידג'טים על רשת של ארבע עמודות: אריחי מספר, תרשימי התפלגות, דונאט ומוערם, קו מגמה, שורות אחרונות, רשימות מובילים, התקדמות ליעד והערות טקסט. בורר תקופה מגביל כל ווידג'ט שיש לו תאריך.
- **List** — הטבלה של ישות אחת (או כרטיסים, לוח, לוח שנה). יכולה להיפתח מסוננת, עם עמודות נבחרות, מיון, תצוגה וגודל עמוד.
- **Tabs** — שניים עד שישה עמודים אחרים זה לצד זה. העמודים המוטמעים בדרך כלל מוסתרים מהניווט כך שיופיעו רק כלשוניות; כרטיס Tabs שואל אילו עמודים להטמיע.
- **Master–detail** — רשימת אב משמאל, שורות הבן של האב הנבחר מימין, מקושרות דרך יחס רבים-לאחד של הבן.
- **Record** — שורה אחת שנפתחת מרשימה, עם הרשימות הקשורות כלשוניות ואריחי מספר מעליהן. לעולם לא בניווט: הוא צריך שורה.
- **Report** — סרגל הסינון של ישות, אחד עד ארבעה תרשימים וסיכומים מקובצים, עם ייצוא CSV כשהאפשרות דולקת.
- **Wizard** — טופס יצירה (ועריכה) מפוצל לשלבים, עם סקירה לפני השמירה.
- **Calendar** — שורות של ישות לפי שדה תאריך: חודש, שבוע, סדר יום או ציר זמן של פסים (ציר הזמן דורש תאריך סיום). נטענת רק התקופה שעל המסך; לחיצה על יום מוסיפה בו שורה.
- **Board** — כרטיסים בנתיבים לפי שדה enum או כן/לא. כל נתיב טוען את הכרטיסים שלו ומציג את הספירה האמיתית; מגבלת נתיב דוחה כרטיסים מעבר לה. מזיזים בגרירה או בתפריט *העברה אל*.
- **Content** — עמוד טקסט: כותרות, רשימות, הערות, **הדגשה**, *נטוי* וקישורים לעמודים אחרים (\`[text](#/page-id)\`) או לרשת. נכתב כטקסט רגיל, ולכן לעולם לא מזריק markup.
- **Import** — העלאת קובץ CSV, התאמת העמודות לשדות ובדיקת כל שורה לפני שדבר נשמר; אז נוצרות כל השורות, או אף אחת. הוא גם מפעיל ייבוא CSV לישות, שרשימתה מקבלת כפתור ייבוא.
- **Search** — תיבת חיפוש אחת על כמה ישויות, התוצאות מקובצות לפי ישות; עם **תיבת חיפוש בכותרת** היא נפתחת מכותרת האפליקציה.

### איך הניווט נבנה
- העמוד הראשון בניווט הוא **עמוד הפתיחה** — האפליקציה נפתחת בו, לכולם. כפתור הבית בשורה הופך עמוד אחר לעמוד הפתיחה.
- עמודים שחולקים **קבוצה** מוצגים יחד כמקטע אחד; רצועת **Navigation** שמעל הרשימה משנה שמות של קבוצות ומסדרת אותן מחדש. **אייקון** הוא אחד מהסט המובנה.
- עמוד **מוסתר** נמצא מחוץ לניווט: רשימה שהוסתרה כך מופיעה רק כלשונית; אשף מוסתר הוא עדיין נתיב.
- **תפקידים** (ADMIN, USER) מגבילים עמוד למשתמשים שמחזיקים באחד מהם. הם דורשים את התלות \`ldap-auth-rest\` (או \`ldap-auth\`) ואינם יכולים לשבת על עמוד הפתיחה או על לשונית — הגבילו את עמוד הלשוניות במקום.

### בעיות ואזהרות
בעיות אדומות חוסמות את Generate: ישות או שדה שכבר לא קיימים, יעד לשונית חסר, מזהה שמשמש פעמיים. **אזהרות** כתומות אינן חוסמות — הן מציינות מבואות סתומים שהאפליקציה הייתה בולעת בשקט: עמוד מוסתר שאף עמוד לא מטמיע, תרשים שלישות שלו אין עמוד רשימה לצלול אליו, שורות אחרונות בלי עמוד רשומה לפתוח, ברירות מחדל שנחתכו. לרובן יש כפתור **Fix**, וכל שינוי ניתן לביטול.

### לעבוד מהר יותר
- שנו את **סוג** העמוד במקום — מה שהסוג החדש לא יכול להשתמש בו מושמט ומצוין.
- התצוגה המקדימה מציירת כל עמוד עם נתוני דוגמה; לחיצה על חלק קופצת לפקד שלו.
- שדות וערכים מוצגים לפי התוויות שלהם, עם השם הגולמי אחריהם כשהשניים שונים.`,
        callouts: [
          {
            type: 'tip',
            text: 'העורך מאמת פריסה בדיוק כפי שהשרת מאמת אותה ב-Generate, כך שסעיף ירוק פירושו שהבקשה תעבור. אם השרת בכל זאת דוחה עמוד, העורך פותח אותו ומדגיש את הבעיה.'
          }
        ]
      },
      {
        id: 'fs-import-ddl',
        title: 'ייבוא מ-DDL',
        description: 'ממלאים את עורך הישויות מראש ע"י הדבקת CREATE TABLE — משתמש בפרסר של אשף ה-SQL הקיים.',
        content: `### מתי להשתמש
כבר יש לכם סכמה (DB קיים, סקריפטי מיגרציה, Excel שאפשר להמיר) ולא רוצים לקליק כל שדה. פותחים את ה-drawer, מדביקים DDL, לוחצים Save — העורך מתמלא בישויות, שדות, טיפוסים, primary keys, אורכים, ודגלי required.

### איך זה עובד
ה-drawer שולח POST ל-\`/metadata/fullstack/import-ddl\` עם ה-SQL והדיאלקט. ה-backend משתמש מחדש ב-\`SqlEntityGenerator.parseTablesForImport\` של אשף ה-SQL הקיים ומריץ דרך מתאם קטן (\`SqlToEntityDefinitionConverter\`) שממפה כל \`TableModel\` ל-\`EntityDefinition\`.

טיפוסים ספציפיים לדיאלקט עובדים פשוט: \`BIGSERIAL\` → Long + generated, \`NUMERIC(p,s)\` → BigDecimal, \`NVARCHAR\` → String, וכו'.

### Replace מול Append
אם בעורך כבר יש ישויות, ה-drawer מציג מתג **Mode** (ברירת מחדל: Append). Replace מנקה את הכל ומשתמש ברשימה שנותחה; Append מוסיף לקיים.

### לוסי בכוונה
המיפוי הוא נקודת התחלה — לבדוק את התוצאה לפני generate. הורדות איכות ידועות:

| מקור | הופך ל | למה |
|---|---|---|
| \`UUID\` (PG), \`UNIQUEIDENTIFIER\` (MSSQL) | \`String\` | אין UUID FieldType ב-v1 |
| \`Short\` | \`Integer\` | אין SHORT FieldType |
| \`Float\` / \`Double\` | \`BigDecimal\` | הכי קרוב לטיפוס מספרי ב-v1 |
| \`byte[]\` (BYTEA / BLOB) | \`String\` | בינארי לא ב-v1 |
| מגבלת \`UNIQUE\` | (לא מוגדר) | UNIQUE ברמת עמודה לא ב-ColumnModel — להחליף ידנית |
| \`CHECK (col IN (...))\` | (לא נותח) | להחליף את הטיפוס ל-Enum ידנית אחרי הייבוא |

### שגיאות פרסור
DDL לא תקין מחזיר HTTP 400 עם \`{error, detail, statementIndex, snippet}\`. ה-drawer מציג את אלה inline כדי שתוכלו לתקן את ההצהרה בלי לאבד את מה שהקלדתם.`,
        codeExamples: [
          {
            title: 'דוגמה — סכמת מסחר קטנה (דיאלקט H2)',
            language: 'sql',
...
CREATE TABLE categories (
    ...
    description VARCHAR(500)
          ...
          {
            title: 'דוגמה — טעם PostgreSQL (BIGSERIAL, UUID, TIMESTAMPTZ)',
            language: 'sql',
    ...
    last_login TIMESTAMPTZ
      ...
      },
      {
        ...
        title: 'תלויות וברירות מחדל פר-סט',
        description: 'איך ה-dep picker מאותחל, ואיך מנהלים מגדירים מה מסומן מראש.',
        content: `### סמנטיקת ה-Wire
התנהגות ה-controller תלויה בשאלה אם השדה \`dependencies\` קיים בגוף הבקשה:

| בקשה | התנהגות |
|---|---|
| **\`dependencies\` חסר** | נופל חזרה לברירות המחדל המוגדרות במנהל של סט ה-backend |
| **\`dependencies: []\`** | מכובד בדיוק — לפרויקט המחולל אין תוספות |
| **\`dependencies: [...]\`** | מכובד בדיוק — בלי מיזוג עם ברירות מחדל, בלי הוספות אוטומטיות |

ה-UI תמיד שולח את הבחירה הסופית של המשתמש מה-picker, אז ביטול סימון של ברירת מחדל באמת מאבד אותה. הכפייה הקודמת של "תמיד להוסיף data-jpa/web/h2" הוסרה — מנהלים מחליטים מה כל סט שולח מסומן מראש, ולמשתמשים יש את המילה האחרונה.

### מאיפה ברירות המחדל מגיעות
כל \`EntityTemplateSetEntity\` (kind=\`BACKEND_JAVA\`) נושא רשימת \`defaultDeps\`, מאוחסנת בטבלה \`entity_template_set_default_dep\`. סט \`spring-jpa-crud\` המקורי מכיל \`["data-jpa","web","h2","validation","actuator"]\` — אותה התנהגות out-of-box כמו קודם, רק שעכשיו אפשר לבטל סימון.

כדי לשנות מה סט שולח מסומן מראש: **Config → Entity CRUD → עריכת הסט → רשימת תיבות הסימון Default Dependencies → שמירה**. השינוי נכנס לתוקף מיד בסשני אשף חדשים (לא צריך \`/admin/refresh\`; אלה לא משפיעים על \`/metadata/client\`).

### בניית סט מתמחה
תבנית נפוצה: לשכפל את \`spring-jpa-crud\` בתור \`spring-jpa-crud-secured\`, להחליף \`h2\` ב-\`postgresql\`, להוסיף \`security\` לברירות המחדל. משתמשים שבוחרים את הסט הזה באשף רואים את התלויות הנכונות מסומנות מראש בלי לחשוב על זה.`,
        callouts: [
          {
            type: 'warning',
            text: 'שום דבר ברשימת התלויות לא חובה למחולל — אבל התבניות המקוריות מפנות ל-`@Entity` (data-jpa), ל-`@RestController` (web), ומניחות DB מובנה אם לא מוגדר datasource. אם משתמש מבטל את אלה, הפרויקט המחולל יקמפל רק אם הוא מביא שווי-ערך משלו.'
          }
        ]
      },
      {
        id: 'fs-template-sets',
        title: 'סטי תבניות',
        description: 'איך הקבצים פר-ישות מאורגנים, ואילו משתנים Mustache חושפת להם.',
        content: `### אנטומיה
סט תבניות הוא חבילה מבוססת-DB של קבצי Mustache עם מפתח \`set_key\` ותגית \`kind\`:

| Kind | מרונדר ע"י | מטרה |
|---|---|---|
| \`BACKEND_JAVA\` | \`EntityScaffoldContributor\` בתוך ה-Initializr pipeline | קבצי \`.java\` פר-ישות; מקבל הרכבת pom ופתרון תלויות בחינם |
| \`FRONTEND_REACT\` | \`FullstackRenderer\` inline | \`package.json\`, \`vite.config.ts\`, עמודי \`.tsx\` פר-ישות, \`Table.tsx\` / \`FormDrawer.tsx\` משותפים |

לכל שורת קובץ יש:
- **pathTemplate** — מרונדר ב-Mustache, אז \`src/main/java/{{packagePath}}/{{EntityName}}Controller.java\` הופך לקובץ אחד פר-ישות.
- **content** — גוף הקובץ, או Mustache (\`substitutionType=MUSTACHE\`) או verbatim (\`NONE\`).
- **perEntity** — \`true\` מרנדר פעם פר-ישות; \`false\` מרנדר פעם בסך הכל (למשל \`App.tsx\`, \`Table.tsx\`).

### Mustache Context
תבניות רואות הקשר מאוחד עם מפתחות ברמת-פרויקט בנוסף לשמות ומטא-דאטה של שדות פר-ישות:

| מפתח | משמעות |
|---|---|
| \`artifactId\`, \`groupId\`, \`packageName\`, \`packagePath\`, \`javaVersion\` | ברמת-פרויקט |
| \`entities\` | רשימה של כל הישויות — ל-\`{{#entities}}…{{/entities}}\` בקבצים לא-perEntity כמו \`AppRoutes.tsx\` |
| \`EntityName\`, \`entityName\`, \`entity_name\`, \`entityNameKebab\`, \`EntityNamePlural\` | וריאנטים של שם פר-ישות |
| \`tableName\` | \`entity.tableName\` או snake_case מרובה של \`EntityName\` |
| \`fields\` | רשימה עם \`name\`, \`Name\`, \`column\`, \`javaType\`, \`tsType\`, \`isString\`, \`isPrimaryKey\`, \`isRequired\`, \`hasLength\`, \`length\`, \`enumValues\` |
| \`stringFields\`, \`hasStringFields\` | מבט מסונן לשרשרת ה-Specification של החיפוש |
| \`pkField\`, \`nonPkFields\` | פיצולי נוחות |
| \`imports\` | רשימת FQN ללא כפילויות לבלוק ה-imports |

### דפדוף / מיון / חיפוש
תבנית ה-Service של \`spring-jpa-crud\` המקורי משתמשת ב-\`{{#hasStringFields}}\` כדי לרנדר שרשרת \`Specification\` עם LIKE על כל עמודות ה-STRING כש-\`?q=\` לא ריק. ישויות בלי שדה STRING נופלות אל \`repository.findAll(pageable)\`. ה-Controller קושר \`?page=\`/\`?size=\`/\`?sort=\` דרך \`Pageable\` של Spring וברירת מחדל \`Sort.by("{pkField.name}").ascending()\`. רכיב ה-Table של React אוסף את ה-props של מיון/חיפוש/דפדוף מהקבצים של \`react-tailwind-crud\` המקורי.`,
        callouts: [
          {
            type: 'info',
            text: 'Mustache ו-JSX משתמשים שניהם ב-`{{` כפותח. תבנית EntityPage המקורית נמנעת מ-`pagination={{ … }}` (ש-Mustache היה מנסה לפרסר כתג) ע"י הוצאת אובייקט ה-pagination ל-`const pagination = { … }` והעברה כ-`pagination={pagination}`.'
          }
        ]
      },
      {
        id: 'fs-admin',
        title: 'ניהול: ניהול סטי תבניות',
        description: 'לשונית Entity CRUD ב-Config — CRUD על סטים, קבצים פר-סט, וברירות מחדל פר-סט.',
        content: `### היכן למצוא
**Config → Entity CRUD**. שני חלוניות:
1. **Sets** (למעלה) — רשימה ראשית, עמודות: \`setKey\`, \`name\`, \`kind\` (BACKEND_JAVA / FRONTEND_REACT), enabled, סדר.
2. **Files in Selected Set** (למטה) — מופיע אחרי לחיצה על שורה ב-Sets. עמודות: \`pathTemplate\`, \`perEntity\`, סוג קובץ, סוג substitution, סדר.

### עריכת סט
ה-drawer חושף את שדות המטא-דאטה הסטנדרטיים בנוסף ל-multi-select של **Default Dependencies** (מוצג רק עבור BACKEND_JAVA). מחפשים ברשימה, מסמנים תיבות, שומרים. מאחורי הקלעים ה-UI שולח POST/PUT לסט ואז PUT למזהי התלויות אל \`/admin/entity-template-sets/{id}/default-deps\` (החלפה מלאה).

### עריכת קובץ
ה-drawer משתמש באותו עורך מבוסס CodeMirror כמו File Contributions. שדה \`pathTemplate\` מרונדר ב-Mustache, אז \`{{packagePath}}\` ו-\`{{EntityName}}\` עובדים כמפרידים. הפעלת \`perEntity\` בוחרת אם הקובץ נפלט פעם פר-ישות או פעם פר-פרויקט.

### ייצוא / ייבוא
\`/admin/export\` מבצע round-trip על \`entityTemplateSets\`, \`entityTemplateFiles\`, ו-\`entityTemplateSetDefaultDeps\`. קובץ הייצוא נייד בין סביבות — שולחים JSON ל-\`/admin/import\` כדי להעביר סט אצור של תבניות.

### זיהוי יתומים
אם מנסים למחוק תלות שמשובצת בברירות המחדל של סט תבניות, זיהוי היתומים הקיים מסמן את זה. \`/admin/cascade-delete\` מסיר את ה-dep ID מברירות המחדל של כל סט גם כן.`
      }
    ]
  },
