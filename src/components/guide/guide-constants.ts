...
Sub-options follow the same \`opts-{depId}=opt1,opt2\` convention as the backend path; the existing \`InitializrWebConfiguration\` servlet filter populates \`ProjectOptionsContext\` for every request that hits the app, so frontend endpoints get sub-options for free.`,
  ...
  // ─────────────────────────────────────────────────────────────────
  // 3. PAIRED GENERATOR (Backend + Frontend Monorepo)
  // ─────────────────────────────────────────────────────────────────
  {
    id: 'paired-generator',
    title: 'Paired Generator',
    icon: 'link',
    topics: [
      {
        id: 'paired-what',
        title: 'What Is the Paired Generator?',
        description: 'One JSON request that produces a Spring Boot backend and a React/Vite frontend, pre-wired so they can talk to each other.',
        content: `### Purpose
The **Paired** tab in the main UI generates **both halves of a stack** in a single request. The response is a monorepo zip with \`backend/\`, \`frontend/\`, a root \`README.md\`, a working \`docker-compose.yml\`, and a \`.menora-init.json\` manifest that describes both halves and lists every file's SHA-256.

This is the unique angle versus \`start.spring.io\` + \`create-vite\`: the same backend can scaffold both halves of a stack, and they're auto-wired to talk to each other on \`localhost\` from the moment you run \`unzip\`.

### Endpoints
- \`POST /starter-paired.zip\` — binary monorepo ZIP
- \`POST /agent/scaffold/paired\` — JSON envelope (utf-8 / base64 file tree + paired manifest), for agents

### Request Shape
\`\`\`json
{
  "backend":  { /* same shape as WizardStarterRequest */ },
  "frontend": { /* PairedFrontendRequest — projectName, deps, apiBaseUrl, ... */ }
}
\`\`\`

Either half can be omitted; the absent side is simply skipped. The dedicated **Backend** and **Frontend** tabs still exist for richer single-project flows (wizards, multi-module, presets, color palettes) — the Paired tab is intentionally minimal.`,
        callouts: [
          {
            type: 'info',
            text: 'The paired endpoint composes the two existing pipelines — it does not introduce a third generator. `PairedStarterController` calls `ProjectGenerationInvoker` for BE then `FrontendProjectGenerator.generateFileMap` for FE, merges both file trees under `backend/` and `frontend/` prefixes, and adds the root files.'
          }
        ]
      },
      {
        id: 'paired-auto-wiring',
        title: 'How the Two Halves Are Auto-Wired',
        description: 'Cross-kind defaults applied automatically when both halves are present.',
        content: `### What Gets Wired
| Wire | Mechanism |
|---|---|
| FE knows the BE URL | \`frontend.apiBaseUrl\` defaults to \`http://localhost:8080\`. Written into \`frontend/.env.development\` as \`VITE_API_BASE_URL\` and into the \`server.proxy\` block of \`vite.config.ts\` so \`/api/*\` forwards to the backend in dev |
| BE accepts the FE origin | A \`paired-cors\` sub-option is auto-added to the BE \`web\` dep — emits \`backend/src/.../config/CorsConfig.java\` allowing \`http://localhost:5173\` against \`/api/**\` |
| Shared OpenAPI contract | If the BE half ships an OpenAPI spec and the FE half selects \`api-client-openapi\`, the same spec lands at \`frontend/openapi.yaml\` (see the next topic) |
| README pairing context | Root \`README.md\` mentions both artifact names and the resolved \`apiBaseUrl\` |

### Defaults & Guard Rails
- The CORS file is only added when \`web\` is in the BE dep set.
- An explicit \`frontend.apiBaseUrl\` overrides the default — useful if you're wiring against a remote dev backend rather than localhost.
- \`FrontendProjectDescription.setApiBaseUrl\` rejects anything that isn't a plain \`http(s)://\` URL with a safe character set, so the value can be spliced into config files without escaping.

### Existing FE Wiring This Builds On
The \`data-tanstack-query\` dep's \`axios-base\` sub-option already ships a baseline \`src/shared/api/axios.ts\` that reads \`import.meta.env.VITE_API_BASE_URL\`. Phase 1 of the paired work just made the BE-paired form fields write that env file — so the existing axios baseline now lights up automatically.`,
        callouts: [
          {
            type: 'tip',
            text: 'The same `apiBaseUrl` / `backendArtifactId` fields exist on `/frontend/starter.zip` as plain query params and in the FE form\'s **Paired Backend** section — you don\'t have to use the Paired tab to wire a frontend to a BE if you only want one half.'
          }
        ]
      },
      {
        id: 'paired-openapi',
        title: 'OpenAPI-Driven Typed Client',
        description: 'Reuse the spec that drives BE codegen to also generate a typed FE client.',
        content: `### The api-client-openapi Dep
The FE catalog has a new **API Integration** group with a single entry: \`api-client-openapi\`. Selecting it adds:
- \`openapi-typescript\` to \`devDependencies\`
- A \`gen:api\` npm script running \`openapi-typescript openapi.yaml -o src/shared/api/generated/schema.ts\`
- A \`src/shared/api/generated/.gitkeep\` placeholder so the output dir exists pre-generation
- A short \`src/shared/api/README.md\` documenting the workflow
- An \`api-client-openapi RECOMMENDS data-tanstack-query\` compatibility rule — the FE banner suggests pairing typed bindings with TanStack Query

### The Spec Itself
The OpenAPI spec only lands at \`frontend/openapi.yaml\` when **both** of these are true:
1. \`api-client-openapi\` is in the FE dep set
2. A non-blank spec is present in \`OpenApiSpecContext\`

In the paired flow, the spec comes from \`backend.specByDep.openapi\` — the same field the BE OpenAPI wizard already uses to drive controller / DTO generation. \`FrontendProjectGenerator\` reads from the shared \`OpenApiSpecContext\` and writes the spec at the FE root.
...
Picking this sub-option emits an \`orval.config.ts\` configured to produce React Query hooks against \`openapi.yaml\`, with the existing \`src/shared/api/axios.ts\` mutator wired in. \`orval\` itself is not auto-installed (no sub-option gating on build customizations today) — users opt in via \`pnpm add -D orval\` and then \`npx orval\`.`,
      ...
      {
        id: 'paired-using',
        title: 'Using the Paired Tab',
        description: 'Two columns, one Generate button, optional OpenAPI spec.',
        content: `### Layout
The Paired tab splits the screen in two:
- **Backend column** — Artifact ID, optional package name, Boot version, and a flat dependency picker drawn from \`/metadata/client\`
- **Frontend column** — Project name, package-manager pill, and a flat dependency picker drawn from \`/frontend/metadata\` (includes the new **API Integration** group)

Below the two columns:
- **API Base URL** input — left blank to default to \`http://localhost:8080\`
- **OpenAPI spec** textarea — paste a spec to drive both BE codegen (existing OpenAPI wizard) and the FE typed client. When filled, the BE \`openapi\` dep is auto-added if absent.

The **Generate Paired Zip** button POSTs the assembled JSON to \`/starter-paired.zip\` and downloads the blob. No localStorage / URL persistence in this MVP; if you want save / share, use the dedicated **Backend** and **Frontend** tabs.

### Manifest Schema
\`MenoraInitManifest\` now has an optional \`paired\` block (sibling to the existing \`inputs\`):
...
\`\`\`json
{
  ...
  "generator": { ... },
  "paired": {
    "backend":    { /* same Inputs shape */ },
    "frontend":   { /* projectName, packageManager, basePath, dependencies, ... */ },
...
Existing single-project manifests are unchanged — \`inputs\` and \`paired\` are mutually exclusive.

### Schema Notes
Phase 2 of the paired work introduced the \`paired-cors\` sub-option on the BE \`web\` dep. Phase 3 added the \`api-client-openapi\` FE dep. Flyway migration \`V7__widen_build_customization_version.sql\` widens \`build_customization.version\` from \`VARCHAR(50)\` to \`VARCHAR(2000)\` — that column is reinterpreted by \`ADD_NPM_SCRIPT\` / \`ADD_VITE_PLUGIN\` as the command / call expression, where 50 chars was too tight for realistic FE scripts.`,
        callouts: [
          {
            type: 'tip',
            text: 'Smoke-test the endpoint without the UI: `curl -X POST http://localhost:8080/starter-paired.zip -H "Content-Type: application/json" -d \'{"backend":{"artifactId":"demo-api","dependencies":["web"]},"frontend":{"projectName":"demo-ui","dependencies":["style-tailwind"]}}\' -o paired.zip`.'
          }
        ]
      },
    ]
