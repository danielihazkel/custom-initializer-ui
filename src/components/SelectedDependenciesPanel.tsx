...
import type { DependencySelectorProps, MetadataOption } from '../types'

type Props = DependencySelectorProps & { compact?: boolean }
...
export function SelectedDependenciesPanel({
  metadata,
  selected,
  onChange,
  extensions,
  ...
  const allDeps = useMemo<MetadataOption[]>(
    () => (metadata?.dependencies?.values ?? []).flatMap(g => g.values ?? []),
    [metadata]
  )

  const depGroupByName = useMemo<Record<string, string>>(() => {
    const map: Record<string, string> = {}
    for (const g of metadata?.dependencies?.values ?? []) {
      for (const d of g.values ?? []) map[d.id] = g.name
    }
    return map
  }, [metadata])
  ...
  const { conflicts, requires } = useDependencyCompatibility(
    ...
    <>
      ...
      >
        ...
        <div className="flex items-center justify-between mb-4 pb-4 border-b border-outline-variant/50">
          ...
          <span className="cut-corners text-[10px] font-mono py-1 px-2.5 bg-primary/15 text-primary">
            {selected.length}
          </span>
        </div>
        ...
        <div className="flex flex-col gap-3 flex-grow overflow-y-auto pr-2 tutorial-scroll">
              ...
              .map(([kind, drivers]) => (
                ...
                >
                  <div className="flex items-center gap-2 mb-3">
                    ...
                    <span className="label-runic-sm text-secondary">{DB_KIND_LABELS[kind] ?? 'Primary Database'}</span>
                  </div>
          ...
          {/* Selected deps list — or empty state */}
          <AnimatePresence mode="popLayout">
            {selectedDeps.length > 0 ? selectedDeps.map(dep => (
              ...
              >
                <div className="flex justify-between items-start gap-2">
                  <div className="flex items-start gap-3 flex-1 min-w-0">
                    ...
                    <span
                      ...
                      style={{ boxShadow: '0 0 8px color-mix(in srgb, var(--color-primary) 70%, transparent)' }}
                    />
                    <div className="flex-1 min-w-0">
                      <h4 className="text-sm font-bold text-on-surface">{dep.name}</h4>
                      {depGroupByName[dep.id] && (
                        <span className="label-runic-sm text-secondary mt-0.5 block" style={{ fontSize: '9px', letterSpacing: '0.28em' }}>
                          {depGroupByName[dep.id]}
                        </span>
                      )}
                    </div>
                  ...
                  <button
                    ...
                    className="text-secondary hover:text-error bg-surface-container hover:bg-error/10 p-1.5 rounded-lg transition-colors flex-shrink-0"
                ...
                {dep.description && (
                  <p className="text-xs text-on-surface-variant leading-relaxed opacity-80 pl-5">{dep.description}</p>
                )}
              ...
              <motion.div
                initial={{ opacity: 0 }}
                animate={{ opacity: 1 }}
                className="flex-1 min-h-[300px] flex flex-col items-center justify-center text-center p-8"
              >
                <span
                  className="material-symbols-outlined text-primary opacity-40 animate-spin-slow"
                  style={{ fontSize: '96px' }}
                  aria-hidden
                >
                  explore
                </span>
                <h4 className="display text-on-surface mt-6 mb-2" style={{ fontSize: '20px' }}>
                  No dependencies yet
                </h4>
                <p className="text-sm text-secondary max-w-[240px]">
                  Pick a Quick Start preset above, or search the Explorer to add dependencies.
                </p>
              </motion.div>
            )}
          </AnimatePresence>
