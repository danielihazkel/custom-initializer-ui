...
import type { FeState } from '../../hooks/useFrontendState'
import type { FePreset } from '../../hooks/useFrontendPresets'

interface PresetPickerFEProps {
  presets: FePreset[]
  recents: FePreset[]
  currentSnapshot: FeState
  onLoad: (snapshot: FeState) => void
  onSave: (name: string, snapshot: FeState) => void
...
function projectLabel(snap: FeState): string {
  const name = snap.form.projectName || 'untitled'
  const scope = snap.form.scope?.trim()
  return scope ? `${scope}/${name}` : name
}

export function PresetPickerFE({
  presets,
  recents,
  currentSnapshot,
  onLoad,
    ...
    <div className="mb-6">
          ...
          >
            <div className="glass-panel rounded-lg flex items-center gap-2 p-3">
              <input
                ...
                placeholder="Preset name (e.g. My dashboard baseline)"
      ...
      {list.length === 0 ? (
        ...
        <div className="flex gap-3 overflow-x-auto pb-2">
          {list.map(item => {
            const depCount = item.snapshot.selectedDeps.length
            return (
              <div
                key={item.id}
