...
describe('the Add-widget gallery helpers', () => {
  it('says which kinds an entity cannot feed, in the validator\'s terms', () => {
    ...
    expect(widgetKindDisabledReason('bar', team)).toBe('Team has no enum or boolean field to break down by')
    expect(widgetKindDisabledReason('donut', team)).toBe('Team has no enum or boolean field to break down by')
